...
|---|---|---|---|---|---|
| 1. Init — Project Bootstrap | 31 | 305 | 192 | 0 | 0 |
...
| 1an | HTTP/2-safe proxied response headers (duplicate singletons, chunked framing, gzip scope) | 1.300-1.305 | complete | 5 | 0 | 0 | a download that started, sat at 0 bytes and ended in a network error, while the proxy log showed only "GET /media/....pdf -> 200". Three defects combined, and none was visible over HTTP/1.1: Date and Server were relayed from the upstream so uvicorn's own copies became DUPLICATES, and both are singleton fields whose repetition an HTTP/2 intermediary rejects by resetting the stream - which the browser reports as ERR_HTTP2_PROTOCOL_ERROR against a 200; Content-Length was dropped from every proxied response, forcing Transfer-Encoding: chunked (forbidden in HTTP/2) and leaving the browser unable to show progress, hence "0 BYTE"; and GZip wrapped the whole app, re-compressing already-compressed PDFs and ZIPs on a loop shared with the Streamlit script runner. The duplication predated the streaming change - verified against the pre-750 proxy, which also emitted two of each - but the streaming change added the chunked framing and the re-compression that made it fatal. 4 of 5 fail against the unfixed tree. NOTE: letters ad-am and scenarios 1.293-1.299 were allocated concurrently by other batches while this work was open; 1an/1.300-1.305 is the first slot free of that. 1.305 added on review: _build_proxied_response treats its two branches differently and 1.300-1.304 only exercised the streaming one, leaving the branch where a stale header is FATAL uncovered - measured, a stale compressed length makes uvicorn raise 'Response content longer than Content-Length' and the client sees http=200 bytes=0, which is the reported symptom verbatim. |
...
| 1u | Fast ASGI health/readiness probes | 1.171-1.175 | complete | 6 | 0 | 0 | scenarios 1.171–1.175; `/health` short-circuits to static liveness, `/readiness` gates on DB readiness, non-probe HTTP falls through to Django |
