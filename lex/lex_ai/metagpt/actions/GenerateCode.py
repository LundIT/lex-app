...
class GenerateCode(Action):
    ...
    async def run(self, project, lex_app_context: str, code: str, class_to_generate, user_feedback, import_pool, stderr=None):
        if not stderr:
            prompt = generate_project_code_prompt_old(project, lex_app_context, code, class_to_generate, user_feedback, import_pool)
        else:
            prompt = regenerate_project_code_prompt(project, lex_app_context, code, class_to_generate, user_feedback, import_pool, stderr)
